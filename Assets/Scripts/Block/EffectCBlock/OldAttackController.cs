...
public class OldAttackController : MonoBehaviour //RAttack
{
    AudioManager AudM;
    AttackManager AttM;
    AttackUI attackUI;
    ...
    List<Block> blockQueue; //待機中の攻撃ブロックのリスト
    ...
    public void Update()
    {
        ...
        //     public async void AddAttackQueue(List<Block> blockList, int lineNum) //消されたブロックを攻撃ブロックとして待機リストに追加
        //     {
        //         doUpdating = true;
    ...
    void CheckEnoughBlocks() //必要数を攻撃ブロックに、余りを待機ブロックに設定
    {
        ...
        //             List<Block> blockList = blockQueue.GetRange(0, needNum);
    ...
    void UpdateWaitingBlock() //待機ブロックを設定
    {
    ...
    bool CanSquire(int blockNum)
    {
        ...
        //     async UniTask SetSquire(List<Block> blockList)
        //     {
        //         int x;
    ...
    void AddWaitingBlock(Block block) //待機ブロックを設定
    {
        // float radius = Mathf.FloorToInt(MathF.Sqrt(attackQueueNum + GetBlockNum()));
    ...
    void ToOneBlock()
    {
        //         foreach(Block block in BlockList)
        //         {
        //             EBlockColor colorType = block.blockColor;
        ...
        //         Block baseBlock = BlockList[0].ReleaseBlock();
        //         float scale = baseBlock.transform.localScale.x * edge;

        //         foreach(Block block in BlockList) BlockPool.ReleaseNotBaseBlock(block);
    ...
    void TextureChange(Block baseBlock)
    {
